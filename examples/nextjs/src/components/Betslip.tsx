...
export function Betslip() {
  ...
  const { items } = useBaseBetslip(); // Keep track of items in the betslip

  return (
    <div className="fixed bottom-4 right-4 w-full max-w-full md:max-w-sm z-50"> {/* Ensure z-index is high */}
      {/* Render the Betslip button even if the betslip is empty */}
      {isOpen && (
        <Content />
      )}
      <button 
        className="flex items-center py-2 px-4 bg-zinc-100 whitespace-nowrap rounded-full ml-auto z-50" 
        style={{ position: 'relative' }} // Adjust the button's position relative to its container
